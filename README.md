...
3. Проверить готовность можно по health-check'ам или пробным запросам:
  ...
  ⚠️ Запуск Legal AI займёт несколько минут: контейнер скачивает веса моделей Ollama (по умолчанию `qwen3-vl:8b-instruct`) и Qdrant-кластеру требуется инициализация.

## Полная пересборка окружения

Иногда требуется полностью очистить окружение и пересобрать все образы и кэши заново. Ниже — последовательность команд, которую удобно выполнять из корня репозитория:

1. Остановите все контейнеры и удалите связанные тома/сети Compose:

   ```bash
   docker compose down --volumes --remove-orphans
   ```

   Если вы запускали только отдельные профили через `make`, команда всё равно корректно остановит и очистит их.

2. Удалите образы, которые собирались в рамках проекта (включая кеш сборки):
   ...
   При необходимости можно удалить все образы/кэш Docker локально: `docker system prune --all --volumes --force` (осторожно, команда очистит весь локальный Docker).

3. Скачайте актуальные версии базовых образов и зависимостей:

   ```bash
   docker compose pull
   ```

4. Пересоберите сервисы без использования кэша:

   ```bash
   docker compose build --no-cache
   ```

   Эквивалент через `Makefile` для полного стека: `make build-ml` (для ML-профиля) и `make build-core` (для остальных сервисов).

5. Запустите проект целиком:

   ```bash
   docker compose up --detach
   ```

   Либо используйте удобные цели Makefile:

   ```bash
   make up-ml   # полный стек (core + ml)
   make up-core # только основные сервисы
   ```

После запуска можно отслеживать прогресс скачивания модели Ollama через логи:

```bash
docker compose logs -f legal-ai-ollama-pull
```

Как только служебный контейнер `legal-ai-ollama-pull` завершится успешно, модель `qwen3-vl:8b-instruct` будет доступна для сервисов `legal-ai` и `contract-extractor`.
