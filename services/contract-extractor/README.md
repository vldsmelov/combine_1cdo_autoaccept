...
⚡ **GPU по умолчанию:** базовый образ собран с PyTorch nightly `cu130`, а Docker Compose запрашивает доступ к NVIDIA GPU (`sm_120`, RTX 5090). При отсутствии GPU Ollama и вспомогательные пайплайны автоматически перейдут в CPU-режим, что замедлит обработку.
...
Такое разделение позволяет кэшировать тяжёлые зависимости и пересобирать только верхний слой при изменениях кода. Базовый слой уже включает PyTorch nightly с поддержкой CUDA 13.0, поэтому дополнительные GPU-библиотеки можно ставить поверх без пересборки низа.
...
2. **Запустите сервисы через Docker Compose**. Укажите уже собранный базовый образ через переменную `API_BASE_IMAGE`, чтобы переиспользовать кэш.
   ...
   Во время запуска `docker compose` автоматически стартует вспомогательный контейнер **`ollama-pull`**. Он дожидается, пока сервис `ollama` пройдёт healthcheck, и инициирует загрузку модели, указанной в переменной `MODEL` (по умолчанию `krith/qwen2.5-32b-instruct:IQ4_XS`). Если модель уже есть в примонтированном томе `ollama_models`, дополнительная загрузка не выполняется.
...
4. **Ручная дозагрузка моделей Ollama**. Если нужно догрузить веса без полного перезапуска стека, выполните команду внутри работающего контейнера `ollama`:
   ```bash
   docker compose exec ollama ollama pull <имя_модели>
   ```
   Команда сохранит модель в томе `ollama_models`, после чего она станет доступна API.
...
- **Уровень логов приложения** задаётся переменной окружения `LOG_LEVEL` (по умолчанию `INFO`). Временное повышение детализации можно сделать прямо при запуске:
  ```bash
  LOG_LEVEL=DEBUG docker compose up
  ```
- **Просмотр текущих логов** выполняется стандартной командой Docker Compose:
  ```bash
