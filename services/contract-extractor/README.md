...
2. **Запустите сервисы через Docker Compose**. Укажите уже собранный базовый образ через переменную `API_BASE_IMAGE`, чтобы переиспользовать кэш.
   ...
   Во время запуска `docker compose` автоматически стартует вспомогательный контейнер **`ollama-pull`**. Он дожидается, пока сервис `ollama` пройдёт healthcheck, и инициирует загрузку модели, указанной в переменной `MODEL` (по умолчанию `qwen3-vl:8b-instruct`). Если модель уже есть в примонтированном томе `ollama_models`, дополнительная загрузка не выполняется.

3. **Изменение модели**. Передайте другое имя модели через `MODEL=...` (например, `MODEL=llama3.1`). По умолчанию сервис запрашивает сжатую версию `qwen3-vl:8b-instruct`, контейнер `ollama-pull` автоматически скачает нужный образ при запуске.
...
Чтобы убедиться, что пайплайн корректно вызывает Qwen3 и сохраняет ответы модели, запустите модульный тест `test_pipeline_llm`:

```bash
python -m unittest discover -s services/contract-extractor/api/tests -p 'test_pipeline_llm.py'
```

Команда создаёт заглушки для внешних зависимостей, импортирует контрактный пайплайн и проверяет, что LLM-конфигурация подтягивается из `CONFIG`, а извлечённые значения попадают в итоговый результат.
